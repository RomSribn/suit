...
class HeaderBar extends React.Component<HeaderBarProps, State> {
  ...
  profileLinkClick = (e: React.MouseEvent) => {
    e.preventDefault();
    const { userStore, isAuth } = this.props;
    if (!isAuth) {
      this.setState({
        showLoginForm: true,
      });
  ...
  render() {
    const { userStore, lang } = this.props;
      ...
      <div className="main__bar">
        <Link
          ...
          className={`profile-link ${
            isRealIndexPage && 'profile-link--index-page-modify'
          }`}
        >
          {!isMobile() && parsedUserName}
        </Link>
        <PopUp open={this.state.showLoginForm} onClose={this.closeForm}>
