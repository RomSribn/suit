    ...
    switch (lang) {
      case 'en':
        return `Elements "${activeItem}" from "${activeSubgroup}" and
         "${accumulateString('exceptionItem', 'en')}" from "${accumulateString('exceptionSubgroup', 'en')}" 
         are mutually exclusive.
         Push the continue button to remove the element "${accumulateString('exceptionItem', 'en')}"
         from the order.`;

      case 'ru':
        return `Элементы "${activeItem}" из подраздела "${activeSubgroup}" 
         и "${accumulateString('exceptionItem', 'ru')}" из подраздела "${accumulateString('exceptionSubgroup', 'ru')}"
