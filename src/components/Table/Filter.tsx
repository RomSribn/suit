...
class Filter extends React.Component<FilterParams> {
  ...
  componentDidUpdate(prevProps: FilterParams) {
    const { updatePickerData } = this.props;
        ...
        return (
          <div
            ...
            className={classNames(
              baseClass,
              baseClass + '--filter',
              baseClass + '--filter_select',
            )}
          >
            <p
              onClick={() =>
                onClickFilterItem!(params, this.inputWrapper, onChange)
              }
              className={classNames('action-text-wrapper', {
                active: isActive,
              })}
            >
