...
export type TableDataItemStringFields =
  | 'id'
  | 'name'
  | 'fitting'
  | 'phone'
...
class Table extends React.Component<TProps, TState> {
  ...
  filterMethod = (
    ...
    const cellValue =
      filter.id === 'status' ? row[filter.id].name : row[filter.id];
    return (
      filter.value === 'all' ||
      cellValue.toLocaleLowerCase().includes(filter.value.toLowerCase())
    );
  };

  triggerShowListPickerFilter = () => {
    const { showListPickerFilter } = this.state;
  ...
  onFilteredChange = (filtered: { id: string; value: string }[]) => {
    this.setState({
      activeFilterValues: filtered.reduce(
        (
          acc: FilterMethodFilter,
          cur: { id: string; value: string; [key: string]: string },
        ) => {
  ...
  updatePickerData = (newfilterData?: FilterData, callback?: () => void) => {
  ...
  datePickerFilterMethod = (
    ...
    if (filter.id === 'date' && filter.value) {
      ...
      if (dateFromStr && dateToStr) {
        ...
        if (
          dateFromMoment.isBefore(curentDate) &&
          dateToMoment.isAfter(curentDate)
        ) {
          return true;
        }
        return false;
  ...
  renderStatusFilter: FilterRender = (props) => {
    const { activeFilterValues, showListPickerFilter } = this.state;
    ...
    return (
      <Filter
        text={
          (activeFilterValues.status &&
            loc[lang].statuses[activeFilterValues.status]) ||
          columns.status
        }
        type="select"
  ...
  renderCell = (props: CellInfo) => {
    ...
    return (
      <>
        <div className="orders__title-mob">
  ...
  render() {
    ...
    const { lang, orders, role } = this.props;
    const { columns, statuses } = loc[lang];
      ...
      <React.Fragment key="Orders table fragment">
        ...
        <RTable
          className={classNames('orders', 'flex-content', {
            orders_blured: showDatePickerFilter,
          })}
          ...
          getTrProps={({}, rowInfo: RowInfo) => {
            return {
              onClick: () => {
                this.panelRow &&
                  this.panelRow.current && // tslint:disable-line
                  // @ts-ignore
              ...
              className: classNames({
                _active: activeOrderId === (rowInfo && rowInfo.row.id),
              }),
            };
          }}
          columns={[
            ...
            },
            {
              ...
              Filter: () => (
                <Filter text={columns.isConfirmed} type="disabled" />
              ),
              filterable: true,
              filterMethod: this.filterMethod,
...
export { Table, TableData, Columns };
