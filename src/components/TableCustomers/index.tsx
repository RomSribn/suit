...
type TableDataItemStringFields =
  | 'name'
  | 'phone'
  | 'email'
  | 'password'
...
class TableCustomers extends React.Component<TProps, TState> {
  ...
  filterMethod = (
    ...
    return (
      filter.value === 'all' ||
      row[filter.id].toLocaleLowerCase().includes(filter.value.toLowerCase())
    );
  };
  ...
  renderCell = (props: CellInfo) => {
    ...
    return (
      <>
        <div className="customers__title-mob">
  ...
  render() {
    ...
    const { lang, customers } = this.props;

    const { columns } = loc[lang];

    return (
      <React.Fragment key="Customers table fragment">
        <PanelRow ref={this.panelRow} lang={lang} />
        <RTable
          ...
          getTrProps={({}, rowInfo: RowInfo) => {
            return {
              ...
              className: classNames({
                _active: activeCustomerId === (rowInfo && rowInfo.original.id),
              }),
            };
          }}
          columns={[
