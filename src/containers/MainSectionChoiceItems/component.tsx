...
const Common = (props: CommonProps) => {
  ...
  const renderClearButton = (choiceItem: SubgroupChoiceItem) => {
    switch (choiceItem.id) {
      case 'initials_text':
        return (
          choiceItem.status &&
          typeof choiceItem.status === 'string' &&
          choiceItem.status !== loc[lang].noStatus
        );
      default:
        return (
          choiceItem.isSubclear !== null &&
          choiceItem.ourCode &&
          choiceItem.ourCode !== choiceItem.defaultCode
        );
    }
  };
  ...
  const commonClasses = classNames('custom', '_design', {
            ...
            <Route
              ...
              component={(...args: any[]) => {
                ...
                return renderClearButton(item) && !isMobile() ? (
                  <span onClick={clearClick(garment, item.id!)}>
                    {loc[lang!].clear}
                  </span>
                ) : null;
              }}
...
class CustomLink extends React.PureComponent<LinkProps> {
  render() {
    ...
    const status =
      isMobile() && !isLandscape() && item.status && item.status!.length > 13
...
class CustomInput extends React.PureComponent<InputProps> {
  ...
  render() {
    const { lang, item, clearClick } = this.props;
    return (
      <Common
        item={item}
        clearClick={clearClick}
...
class ChoiceItems extends React.PureComponent<ChoiceItemsProps> {
  ...
  render() {
    const { items, basicRoute, lang, visitedChoiceItems } = this.props;
    return (
      ...
      >
        {items.map((item: SubgroupChoiceItem) =>
          item.isInput ? (
            <CustomInput
              lang={lang}
              item={item}
