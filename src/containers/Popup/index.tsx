...
class PopUp extends React.Component<IncomingProps> {
  ...
  render() {
    const { containerSelector, open, onClose, children } = this.props as Props;
    if (!containerElement) {
      const domNode = document.querySelector(containerSelector);
      if (!domNode) {
        throw Error(
          `There is no container for popup-portal provided by ${containerSelector} selector`,
        );
      }
    ...
    return ReactDOM.createPortal(
      <div
        className={classNames('popup__container', { open: open })}
        onClick={onClose}
      >
        <span
          onClick={(e) => {
