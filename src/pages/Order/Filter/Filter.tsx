...
class FilterItem extends React.PureComponent<FilterItemProps> {
  ...
  onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { value, addFilter, removeFilter } = this.props;
    if (e.target.checked) {
      addFilter(value);
    } else {
      removeFilter(value);
  ...
  componentDidUpdate(prevProps: DefaultFilterItemProps) {
    if (!isEqual(prevProps, this.props)) {
      ...
      const isChecked =
        userFilters[group] &&
        userFilters[group].find((item: string) => item === value);
  ...
  render() {
    const { valueTitle, lang, type, value, group, isColorGroup } = this.props;
    const { checked } = this.state;
    return (
      <label className={classnames('filter__label', { color: isColorGroup })}>
        <input
          type={type!}
          value={value}
          name={group}
