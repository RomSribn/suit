...
export const updateOrder = (props: Props) => {
  try {
    if (props.match) {
      ...
      if (subgroup !== 'initials_text') {
        if (subgroup === 'fabric') {
          if (orderStore!.partOfShirtToggle === basisPart) {
            ...
            newValue[garment][0][group][
              subgroup
            ].our_code = orderStore!.activeElement!.our_code;
            newValue[garment][0][group][
              subgroup
            ].title = orderStore!.activeElement!.title;
          } else {
            // устанавливаем доп ткань для части рубашки из активного элемента
            newValue[garment][0].design[
              orderStore!.partOfShirtToggle
            ].additionalFabric = orderStore!.activeElement!.our_code;
          }
        } else {
          if (!newValue[garment][0][group][subgroup]) {
      ...
      const subgroupData = new Subgroups(garment).data.design.find(
        (category: Subgroup) =>
      ...
      Object.keys(defaultValues!)
        ...
        .map((filteredGarment) =>
          defaultValues![filteredGarment]
            .reduce((ac: string[], i: string[]) => [...ac, i], [])
            .map((garmentObject: Order) =>
      ...
      if (orderStore!.order) {
        Object.keys(orderStore!.order)
          .filter((item) => !item.includes('manequin'))
          .map((filteredGarment) =>
            orderStore!.order[filteredGarment]
              .reduce((ac: string[], i: string[]) => [...ac, i], [])
              .map((garmentObject: Order) =>
                Object.keys(garmentObject.design).forEach((elementKey) => {
                  if (garmentObject.design[elementKey]) {
                    orderItems[elementKey] =
                      garmentObject.design[elementKey].our_code;
                  } else {
          ...
          Object.keys(exceptions[garmentKey]).forEach((elementKey) => {
            if (
              ...
              allExceptions.forEach((exceptionCode) => {
                const isExceptionItemExistInOrder = arrayItemsInOrder.find(
                  (key: string) =>
