...
const makeCatalogItems: MakeCatalogItems = (
  ...
  Object.keys(garments).map((garment) => {
    ...
    const firstCurrentActiveGarment =
      currentActiveGarment[0] || activeGarments[0];
...
class GarmentChoise extends React.Component<GarmentChoiceFormProps, State> {
  ...
  componentDidMount() {
    ...
    const commonHeight = Array.from(DOMElemtnt.children).reduce(
      (acc, child) => {
  ...
  makeOrder = (e: React.MouseEvent<HTMLAnchorElement>) => {
    const { activeGarments, pushOrderPathitem, lang } = this.props;
      ...
      >
        ...
        >
          {isRealIndexPage() && (
            <CatalogIntroText
              lang={lang!}
              isClearSubmit={this.state.isClearSubmit}
            />
          )}
          <div
            className={`catalog__form-wrap${
              isRealIndexPage() ? ' index-mobile' : ''
            }`}
          >
            {makeCatalogItems(
              garments!,
            ...
            {!isRealIndexPage() && <GarmentViewController />}
          </div>
