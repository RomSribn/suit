...
class OrderInfo extends React.PureComponent<OrderInfoProps> {
  ...
  render() {
    const { lang, deliveryDate, price } = this.props;
    const fullDeliveryDate =
      isTablet() && !isLandscape()
        ? deliveryDate
            .split('.')
            .slice(0, deliveryDate.split('.').length - 1)
            .join('.')
        : deliveryDate;
    return (
      <div className="calc-blc">
        <div className="calc-blc__col calc-blc__col--summ">
          ...
          <div className="calc-blc__col-val">
            <div className="calc-blc__col-val__curr">{loc[lang!].currency}</div>
            {price}
          </div>
        </div>
    ...
    return (
      <div className="calc-blc-mobile">
        <span className="calc-text-mobile">
          ...
          <span className="portrait-hide">
            &nbsp;/&nbsp; {`${loc[lang].delivery} ${deliveryDate}`}
          </span>
        </span>
      </div>
    );
...
export { OrderInfo, OrderInfoMobile };
