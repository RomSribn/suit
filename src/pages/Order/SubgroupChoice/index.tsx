...
export const filterFields: FilterFields = (
  ...
  try {
    ...
    if (item.is_input) {
      status =
        typeof orderVal[subgroup][item.subsection_our_code] === 'object'
          ? ''
    ...
    if (order[garment][0].design[item.subsection_our_code]) {
      ...
      defaultCode = defaultValues!.fabric_ref![item.subsection_our_code]
        .our_code;
    }
  } catch (_) {
...
class SubgroupChoice extends React.Component<SubgroupChoiceProps> {
  getSnapshotBeforeUpdate(prevProps: SubgroupChoiceProps) {
    ...
    if (
      (action !== 'POP' && pathname.includes('design')) ||
    ...
    const data = $store
      ? [
          ...$store.design.map((v: Subgroup) =>
            filterFields(v, 'design', lang!, order!, garment, defaultValues),
          ),
        ]
      : [];
    ...
    const dataFitting = $store
      ? [
          ...$store.fitting.map((v: Subgroup) =>
            filterFields(v, 'fitting', lang!, order!, 'shirt', defaultValues),
          ),
        ]
      : [];
              ...
              <>
                ...
                <div className={'nav-overflow'}>
                  <div className={'design-navigation-wrapper'}>
                    <Link
                      to={match.url}
                      className={'design-navigation _active'}
                    >
                      Все
                    </Link>
                    {data.map((item) => (
          ...
          render={(props) => {
            ...
            return (
              <>
                ...
                <div className={'design-navigation-wrapper'}>
                  <Link to={match.url} className={'design-navigation _active'}>
                    Все
                  </Link>
