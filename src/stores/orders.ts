...
class OrdersStore implements OrderList.IOrderStore {
  ...
  updateOrder = (order: OrderList.OrderItem) => {
    return callApi(
      ...
      () => {
        const currentOrderIndex = this.orders.findIndex(
  ...
  deleteOrder = (orderId: number) => {
    return callApi(
      ...
      (response) => {
        this.orders = observable.array<OrderList.OrderItem>(
          this.orders.filter((order) => order.orderId !== orderId),
        );
        this.isFetching = false;
      },
      this._onError,
      ...
      () => {
        ...
        if (currentOrderIndex !== -1) {
          this.orders[currentOrderIndex] = {
            ...
            // @ts-ignore
            customer: {
              ...this.orders[currentOrderIndex].customer,
              isConfirmed: true,
            },
          };
        }
...
export { instanse as ordersStore };
